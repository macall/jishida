...
class Sdo_payment implements payment{
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		if ($bank_id=='0' || strim($bank_id) == 'SDO1' || strim($bank_id) == 'SDO'){
			$bank_id = '';
		}		
		...
		$array=array(
			...
			'SendTime'=>to_date(NOW_TIME,'YmdHis'),
			'OrderTime'=>to_date(NOW_TIME,'YmdHis'),
			'PayType'=>'PT001',
	...
	public function get_display_code(){
		...
		$payment_cfg = unserialize($payment_item['config']);
 		if($payment_item)
		{
	        $def_url = "<style type='text/css'>.bank_sdo_types{background:url(".SITE_DOMAIN.APP_ROOT."/system/payment/SdoBank/banklogo.gif) no-repeat;}";
	        
	        $def_url .=".bk_typeSDO1{background-position:15px -1414px; }";    //盛付通
			$def_url .=".bk_typeSDTBNK{background-position:15px -5px; }";    //测试银行
			$def_url .=".bk_typeICBC{background-position:15px -46px; }";    //工商银行
	        ...
	        {
	            $def_url  .= "<label class='bank_sdo_types bk_type".$key." ui-radiobox' rel='payment_rdo'><input type='radio' name='payment' value='".$payment_item['id']."' rel='".$key."' onclick='set_bank(\"".$key."\")' /></label>";
	        }
