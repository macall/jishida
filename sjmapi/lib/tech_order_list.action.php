...
class tech_order_list{
	public function index()
	{
		...
		if($user_id>0)
		{
			...
			if ($totalRows == 0){		
				$totalRows = $GLOBALS['db']->getOne("select count(*) from ".$order_table_name." where technician_id = ".$user_id." and type = 0 and is_delete = 0 {$pay_status_and}");	
			...
			$list = $GLOBALS['db']->getAll("select * from ".$order_table_name." where technician_id = ".$user_id." and type = 0 and is_delete = 0  order by create_time desc limit ".$limit);		
			...
			{
				$orderlist[] = get_order_goods2($item);
			}
