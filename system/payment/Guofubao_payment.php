...
class Guofubao_payment implements payment {
    ...
    public function get_payment_code($payment_notice_id) {
        ...
        */
        //新
        $data_front_url =  SITE_DOMAIN.APP_ROOT.'/callback/payment/guofubao_callback.php?act=notify';
        //$data_front_url="";
        $data_return_url = SITE_DOMAIN.APP_ROOT.'/callback/payment/guofubao_callback.php?act=response';
        ...
        $gopayServerTime = strim(file_get_contents("https://www.gopay.com.cn/PGServer/time"));
        ...
        $def_url .= "<button class='ui-button paybutton' rel='blue' type='submit'>前往国付宝在线支付</button>";
        $def_url .= "</form>";
			...
			if($rs)
			{
				...
				if($rs)
				{
					...
					if($order_info['type']==0)
						echo "RespCode=0000|JumpURL=".SITE_DOMAIN.url("index","payment#done",array("id"=>$payment_notice['order_id'])); //支付成功
					else
						echo "RespCode=0000|JumpURL=".SITE_DOMAIN.url("index","payment#incharge_done",array("id"=>$payment_notice['order_id'])); //支付成功
				}
				else 
				{
					...
					{				
						...
						echo "RespCode=0000|JumpURL=".SITE_DOMAIN.url("index","payment#pay",array("id"=>$payment_notice['id'])); 
				}
			}
			else
			{
    ...
    public function get_display_code() {
		...
		if($payment_item)
		{
	        ...
	        $html = "<style type='text/css'>.guofubao_types{ background:url(".SITE_DOMAIN.APP_ROOT."/system/payment/Guofubao/banklist_hnapay.jpg); }";
	        $html .=".bk_type_CCB{background-position:15px -78px; }"; //中国建设银行
	        $html .=".bk_type_CMB{background-position:15px -202px; }"; //招商银行
	        $html .=".bk_type_ICBC{background-position:15px 0px; }"; //中国工商银行
	        $html .=".bk_type_BOC{background-position:15px -119px; }"; //中国银行
	        $html .=".bk_type_ABC{background-position:15px -40px; }"; //中国农业银行
	        $html .=".bk_type_BOCOM{background-position:15px -164px; }"; //交通银行
	        $html .=".bk_type_CMBC{background-position:15px -236px; }"; //中国民生银行
	        $html .=".bk_type_HXBC{background-position:15px -364px; }"; //华夏银行
	        $html .=".bk_type_CIB{background-position:15px -276px; }"; //兴业银行
	        $html .=".bk_type_SPDB{background-position:15px -318px; }"; //上海浦东发展银行
	        $html .=".bk_type_GDB{background-position:15px -484px; }"; //广东发展银行
	        $html .=".bk_type_CITIC{background-position:15px -402px; }"; //中信银行
	        $html .=".bk_type_CEB{background-position:15px -441px; }"; //光大银行
	        $html .=".bk_type_PSBC{background-position:15px -519px; }"; //中国邮政储蓄银行
	        $html .=".bk_type_SDB{background-position:15px -564px; }"; //深圳发展银行
	        $html .="</style>";
	       ...
	       foreach ($payment_cfg['guofubao_gateway'] AS $key=>$val)
	        {
	            $html  .= "<label class='guofubao_types bk_type_".$key." ui-radiobox' rel='payment_rdo'><input type='radio' name='payment' value='".$payment_item['id']."' rel='".$key."' onclick='set_bank(\"".$key."\")' /></label>";
	        }
